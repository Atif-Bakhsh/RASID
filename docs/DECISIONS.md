...
| Optional AI analyst   | On-demand OpenAI Responses call over allow-listed aggregate demo facts; strict JSON and evidence validation; disabled by default | Fact-boundary, failure, HTTP auth/validation, and frontend interaction tests |
...
No bank integration, real financial data, money movement, investment execution, live market feed, required AI dependency, queues, Redis, or microservices. The optional AI analyst is presentation-only: deterministic calculations remain authoritative and the product remains usable when it is disabled or unavailable.
